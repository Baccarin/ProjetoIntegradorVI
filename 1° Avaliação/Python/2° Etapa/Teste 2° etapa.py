...
try:
    connection = mysql.connector.connect(
        host='localhost', database='PIVI', user='root', password='30252115')
    ...
    insertSistema = """insert into sistema (nome,arquitetura)
        values (%s,%s);"""
    if (plat.system() == 'Windows'):
        nome = 'windows'
    else: 
        nome = 'outro'
    arquitetura = (platform.architecture().__getitem__(0))
    values = (nome,arquitetura)   

    cursor.execute(insertSistema,values)
    print("sistema")
    ...
    values = (nome, processador)
    cursor.execute(insertMaquina, values)
    print("maquina")
